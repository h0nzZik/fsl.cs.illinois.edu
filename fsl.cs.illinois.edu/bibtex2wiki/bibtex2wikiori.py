...
class Paper(object):
  def __init__(self, id, year, entry):
    ...
    for category in self.category:
      if (not category in categories):
        print_warning("category " + category + " not found; add it to the "
    ...
    try:
      ...
      try:
        ...
        print_warning("invalid month " + entry.fields["month"] )
        mm = 1
    except KeyError:
      mm = 1
    ...
    try:
      ...
      print_error("invalid year " + entry.fields["year"])
      yyyy = 1900
    ...
    if (path.isfile(pdf_file)):
      ...
      if (path.isfile(public_pdf_file)):
        ...
        if (not args.action == "header"):
          print_warning("missing header in " + get_pdf(id) + " document;" 
              + " click the 'add header' button to add it")
    else:
      self.public_pdf = ""
      print_warning("missing " + get_pdf(id) + " file")
    ...
    if (path.isfile(reviews_file)):
      ...
      if (self.status == "submitted"):
        print_warning("missing " + get_reviews(id) + " file")
    ...
    if (path.isfile(bib_file)):
      ...
      self.bib = ""
      print_warning("missing " + get_bib(paper_id + "-ref") + " file")

  def add_field(self, entry, name):
    try:
      setattr(self, name, entry.fields[name])
    except KeyError:
      raise FieldError("missing " + name + " field")
  ...
  def make_pdf(self):
    ...
    if (path.isfile(paper_tex)):
      ...
      print_error("no such .tex file " + paper_tex)
      return
  ...
  def add_header(self):
    ...
    if (path.isfile(paper_pdf)):
      ...
      print_error("no such .pdf file " + paper_pdf)
      return
  ...
  def get_id_year(self):
    ...
    try:
      ...
      print_error("unexpected format of paper id " + self.id)
      return 1900
...
class ConferencePaper(Paper):
  ...
  def __init__(self, id, year, entry):
    ...
    try:
      # series field supersedes publisher field
      self.publisher = entry.fields["series"]
    except KeyError: 
      try:
        # default to publisher field
        self.publisher = entry.fields["publisher"]
      except KeyError: 
        # unknown publisher
        self.publisher = ""
        print_warning("missing " + "publisher/series" + " field")
    ...
    if (self.publisher != ""
      ...
      print_warning("unrecognised publisher/series " + self.publisher)
    ...
    if (self.publisher == "LNCS" or self.publisher == "ENTCS"):
      if (self.volume == ""):
        print_warning("missing volume information for LNCS/ENTCS publication")
    self.add_optional_field(entry, "pages", warning=True)
  ...
  def get_presentation_links(self):
    if (not self.status == "published" or self.presentation == ""):
      return []
    ...
    if (presentation_links == []):
      print_warning("no presentation found")
...
def get_paper(bib_data, year_dir, paper_id):
  try:
    ...
    if (paper_entry.type == ConferencePaper.type):
      ...
      print_error("unsupported bib entry " + paper_entry.type)
      return None
